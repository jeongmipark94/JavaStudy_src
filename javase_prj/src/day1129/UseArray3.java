...
public class UseArray3 {
	...
	public UseArray3() {
		// 3차원 배열 참조형 형식의 사용.
		// 1.선언 : 데이터형[][][] 배열명 = null; 데이터형 배열명[][][]= null; -> 좋지 않음
		int[][][] arr = null;
		...
		System.out
				.println("arr1배열의 면의 수 : " + arr1.length + " ,행의 수 :" + arr1[0].length + "열의 수 : " + arr1[0][0].length);
		// 5. 모든 방의 값 출력)
		for (int i = 0; i < arr.length; i++) {// 면
			System.out.println(i + "면 시작");
			for (int j = 0; j < arr[i].length; j++) { // 행

				System.out.println(i + "면" + j + "행시작");
	...
	public static void main(String[] args) {
		new UseArray3().priType(); // 생성자지만 .찍어서 메소드 부르는걸 메소드 체인이라고 한다.

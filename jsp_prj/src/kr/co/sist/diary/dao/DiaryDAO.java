...
public class DiaryDAO {
	...
	/**
	 * 년,월을 입력받아 해당 월의 모든 일자의 글 번호, 제목을 가변배열에 저장하여
	 * 반환하는 일을 하는 메서드
	 * @param year 년
	 * @param month 월
	 * @return 가변배열
	 * @throws SQLException
	 */
	public MonthVO[][] selectMonthEvent(String year,String month) 
				throws SQLException{
		...
		//5.
			...
			for(int i=0; i<32; i++) {//해당 년월의 
				pstmt.setString(3, String.valueOf(i+1 ));//1일 부터 31일까지 쿼리를 수행
				rs=pstmt.executeQuery();
				while(rs.next()) {//실행 결과가 존재한다면 해당 일자에 이벤트 글이 존재하므로 
					//이벤트 글의 값을 저장한다.
	...
	/**
	 * 글번호, 내용, 비밀번호를 입력받아 비밀번호가 일치하면 해당 글번호의 이벤트 내용을
	 * 변경하는 일.
	 * @param du_vo
	 * @return
	 * @throws SQLException

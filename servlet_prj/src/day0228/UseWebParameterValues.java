...
public class UseWebParameterValues extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		response.setContentType("text/html;charset=UTF-8");//접속자에게 보내줄 형식을 만듬, 이 파일은 접속자의 웹브라우저가 실행한다.
		PrintWriter out = response.getWriter();
	      ...
	      out.print("\t<strong>");
	      out.print(name);
		  out.print("</strong>님 께서 입력하신 취미는 아래와 같습니다. <br/>");
		  
		  out.println("\t<ul>");
		  try {//체크박스가 선택된 것이 없다면 hobby 배열에는 null이 할당되므로
			  //length 키워드를 사용하게 되면 NullPointerException이 발생하게된다.

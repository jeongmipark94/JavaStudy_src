 ...
 * 접속자가 존재하면 접속자 소켓에 얻어낸 스트림에서 메세지를 무한 루프로 읽어들이고 모든 접속자에게 메세지 출력
 * 
 * @author owner
 */
public class MultiChatServerHelper extends Thread {
	...
	public MultiChatServerHelper(Socket socket, DefaultListModel<String> dlm, int cnt, JFrame jf,
		...
		someClient = socket; // 매개변수의 이름이 달라서 앞에 this 안써줘도 됨. 식별이 가능하니까
		this.dlm = dlm;
		this.cnt = cnt;
		this.jf = jf;
		connectList = list;
	...
	public synchronized void broadcast(String msg) {
		...
		try {
			for (int i = 0; i < connectList.size(); i++) {
				...
				mcsh.writeStream.writeUTF(msg); // 출력 스트림에 출력하고

	...
	public void run() {
		...
		try {
			try {
				...
				List<String> tempFileList = new ArrayList<String>(); //list 생성 서버,클라이언트 파일 비교
...
//						
//						if(flag == false) {
//							tempArr.add(serverFileNames[i]);
//						}
//						
//					}
//					System.out.println(tempArr);
					
//					if((Arrays.toString(serverFileNames)!=Arrays.toString(fileNames))) {
//						String[] arr = (Arrays.toString(serverFileNames))- (Arrays.toString(fileNames));
//						System.out.println(arr);
//					}//end if
					//ㄴ> 이렇게는 안됨. 문자열을 -로 엮을 수가 없음
...
//					System.out.println("클라이언트 " + Arrays.toString(fileNames) );
	...
	private void fileSend(String fName, DataOutputStream dos ) throws IOException{
		...
		try {
			...
			int fileLen = 0;
			while( (fileLen=fis.read(readData)) !=-1) {///////////////////////????????????=이 왜 2개인거지
				fileData++;
			}//end while
			fis.close();
